...
export async function getAvailability(date: Date, now = new Date()) {
  ...
  return slots.map((slot) => ({
    ...
    state: appointments.some((a) => a.startsAt < slot.endsAt && a.endsAt > slot.startsAt) ? "reserved" : "available",
  }));
}
